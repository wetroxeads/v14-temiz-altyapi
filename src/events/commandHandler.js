...
module.exports = async (message) => {
  let prefix = setting.prefix.find((x) => message.content.toLowerCase().startsWith(x));
  if (message.author.bot || !message.guild || !prefix) return;
  let args = message.content.substring(prefix.length).trim().split(" ");
  let commandName = args[0].toLowerCase();

  const embed = new EmbedBuilder().setFooter({ text: setting.Footer}).setAuthor({ name: message.member.displayName, iconURL: message.author.avatarURL({ dynamic: true })});
  ...
  if (cmd) {
    if (cmd.conf.owner && !setting.Owners.includes(message.author.id)) return;
    ...
    `)
    if(komutLog) komutLog.wsend({ embeds: [vale]})
  }
};
